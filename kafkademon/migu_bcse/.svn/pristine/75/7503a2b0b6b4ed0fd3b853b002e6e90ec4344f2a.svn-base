...
/**
 * copy了优先队列的实现，将其中的private方法改成了public
 * 
 * @author Josh Bloch, Doug Lea
 * @param <E>
 *            the type of elements held in this collection
 */
public class PriorityQueue1<E> extends AbstractQueue<E>implements java.io.Serializable {
	 ...
	 *             if {@code initialCapacity} is less than 1
	 */
	public PriorityQueue1(int initialCapacity, Comparator<? super E> comparator) {
	...
	public PriorityQueue1(Collection<? extends E> c) {
		...
		} else if (c instanceof PriorityQueue1<?>) {
			PriorityQueue1<? extends E> pq = (PriorityQueue1<? extends E>) c;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public PriorityQueue1(PriorityQueue1<? extends E> c) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public PriorityQueue1(SortedSet<? extends E> c) {
		this.comparator = (Comparator<? super E>) c.comparator();
		initElementsFromCollection(c);
	}

	private void initFromPriorityQueue(PriorityQueue1<? extends E> c) {
		if (c.getClass() == PriorityQueue1.class) {
	...
	private final class Itr implements Iterator<E> {
		...
		public void remove() {
			if (expectedModCount != modCount)
				throw new ConcurrentModificationException();
			if (lastRet != -1) {
				E moved = PriorityQueue1.this.removeAt(lastRet);
				lastRet = -1;
				if (moved == null)
					cursor--;
				else {
				...
				PriorityQueue1.this.removeEq(lastRetElt);
				lastRetElt = null;
			} else {
