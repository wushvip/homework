...
public class BCSEElevate {
	...
	private String path="/config/spread";
	
    public BCSEElevate(BCSEClient client){
    	this.client=client;
	}
	...
	private Map<String, Object> extract() {
		...
		if(StringUtil.isNotEmpty(getSpreadIds())){
			map.put("spreadIds", getSpreadIds());
		}
		return map;
	}
}
